    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['edu:authority:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['edu:authority:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listAuthority, getAuthority, delAuthority, addAuthority, updateAuthority, exportAuthority } from "@/api/edu/authority";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAuthority(authorityId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除认证机构编号为"' + authorityIds + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有认证机构数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
